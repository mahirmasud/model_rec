...
# Autonomous RecBole pipeline (auto schema detection + training + recommendation export)
...
RecBole mode now also persists recommendation output automatically:
- `outputs/top_k_recommendations.parquet` (or your configured `data.output_dir`)
- Includes top-k recommendations for all detected users from the cleaned interaction dataset.
...
- Uses an autogenerated RecBole default of `epochs: 50` unless overridden.
